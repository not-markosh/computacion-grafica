...
const float SIDE_LONG = 2.0f;
const float SIDE_SHORT = 2.0f;
...
void Animation() {
	...
	if (dogAnim == 1)
	{
		bool isWalking = (pathState == 0 || pathState == 2 ||
			pathState == 4 || pathState == 6 ||
			pathState == 8);
		...
		float diagonalDist = sqrt(SIDE_LONG * SIDE_LONG + SIDE_SHORT * SIDE_SHORT);

		switch (pathState)
		{
		case 0:
			dogPos.x += WALK_SPEED * sin(radRot);
			dogPos.z += WALK_SPEED * cos(radRot);
			distanceWalked += WALK_SPEED;
			if (distanceWalked >= SIDE_SHORT)
			{
				distanceWalked = 0.0f;
				pathState = 1;
			}
		...
		case 1:
			dogRot -= TURN_SPEED;
			rotationProgress += TURN_SPEED;
			if (rotationProgress >= 90.0f)
			{
		...
		case 4:
			...
			dogPos.z += WALK_SPEED * cos(radRot);
			distanceWalked += WALK_SPEED;
			if (distanceWalked >= 2.0f * SIDE_LONG)
			{
				distanceWalked = 0.0f;
				pathState = 5;
			}
		...
		case 6:
			...
			dogPos.z += WALK_SPEED * cos(radRot);
			distanceWalked += WALK_SPEED;
			if (distanceWalked >= 2.0f * SIDE_SHORT)
			{
				distanceWalked = 0.0f;
				pathState = 7;
			}
		...
		case 8:
			...
			dogPos.z += WALK_SPEED * cos(radRot);
			distanceWalked += WALK_SPEED;
			if (distanceWalked >= diagonalDist)
			{
				distanceWalked = 0.0f;
				pathState = 9;
			}
